...
#ifndef TRAINANDTICKET_BPLUSTREEFORTICKET_H
#define TRAINANDTICKET_BPLUSTREEFORTICKET_H
//
...
bool erase(const Key &key){
    ...
    //if(key == debug) cout << "fuck0" << endl;
    if(leaf.numOfpair < LEAST_PAIR_IN_LEAF_TICKET){
        //if(key == debug) cout << "fuck0" << endl;
...
bool find(const Key &key, Key k[] , Value v[], int &num){
    long long offset = 0;
    offset = find_in_Key(key, meta.root);
    //offset = meta.firstleaf;
    ...
    if(offset == 0){
        ...
        //int x = leaf_binary_search_range(leaf.children, 0, leaf.numOfpair - 1, key);
        int x = 0;
        bool found = false;
        while(true){
            for(int i = 0; i <= leaf.numOfpair - 1; i++){
                if(keycmp(leaf.children[i].first, key) == 0) {
                    found = true;
                    x = i;
                    //std::cout << "the fucking place " << leaf.offset << std::endl;
        ...
        /*while(x == 0){
            std::cout << "come to find " << std::endl;
            readfile(&leaf, leaf.next, 1, sizeof(leaf_node<Key, Value>));
            for(int i = 0; i <= leaf.numOfpair - 1; i++){
                if(keycmp(leaf.children[i].first, key) == 0) {
                    x = i;
                    std::cout << "here is my day" << std::endl;
                    break;
                }
            }
        }*/
        ...
        num = 0;
        bool flag = false;
        while(keycmp(leaf.children[x].first, key) == 0){
            flag = true;
            k[num] = leaf.children[x].first;
            v[num] = leaf.children[x].second;
            num++;
            x++;
            if(x == leaf.numOfpair) {
                x = 0;
...
long long find_in_Key(const Key &key, long offset){
    internal_node<Key, Value> node;
    readfile(&node, offset, 1, sizeof(internal_node<Key, Value>));
    if(node.type == 1){//means it is going to the leaf
        int r = node.numOfkey - 1;
        int l = 0;
        //int x = node_binary_search_range(node.index, l, r, key);
        int x = 0;
        bool found = false;
        for(int i = 0; i <= r; i++){
            /*if(keycmp(node.index[i].first, key) == 0){
                x = i - 1;
                //std::cout << "very good" << x <<std::endl;
                break;
            }*/
            if(keycmp(node.index[i].first, key) != 1) {
                found = true;
                x = i - 1;
                if(x < 0){
                    x = 0;
                }
                //std::cout << "good" << x << std::endl;
                break;
            }
        }
        if(!found) x = r;
        ...
        //std::cout << "find x to leaf " << x  << " " << node.index[x].second << std::endl;
        ...
        if(!found) x = r;
        //std::cout << "find x in node " << x << std::endl;
        //std::cout << find_leaf(key, node.index[x].second) << std::endl;
        //std::cout << "fuck" << std::endl;
        //std::cout << find_in_Key(key, node.index[x].second) << std::endl;
        return find_in_Key(key, node.index[x].second);
    }
}



int node_binary_search_range(const pair<Key, long> p[], int l, int r, const Key &key){
    int ans = 0;
    if(l > r){
        return 0;
    }
    if(keycmp(p[r].first , key) == 1){
        return -1;
    }
    if(keycmp(p[l].first, key) == -1){
        return -1;
    }
    while(l <= r){
        int mid = (l + r) / 2;
        if(keycmp(p[mid].first , key) != 1){
            ans = mid;
            r = mid - 1;
        }
        else{
...
int leaf_binary_search_range(const pair<Key, Value> p[], int l, int r, Key key){
    int ans = 0;
    if(l > r){
        return 0;
    }
...
long find_in_Key(const Key &key, long offset){
    internal_node<Key, Value> node;
    readfile(&node, offset, 1, sizeof(internal_node<Key, Value>));
    if(node.type == 1){//means it is going to the leaf
        int r = node.numOfkey - 1;
...
int node_binary_search_range(const pair<Key, long> p[], int l, int r, Key key){
    int ans = 0;
    if(l > r){
        throw "false";
    }
...
bool borrow_from_rightleaf(leaf_node<Key, Value> &leaf){//向右边叶子借东西
    ...
    if(rightleaf.numOfpair <= LEAST_PAIR_IN_LEAF_TICKET){//如果右边叶子贫穷，那么就媒介成功
        return false;
    } else{
        //borrow from right
...
bool borrow_from_leftleaf(leaf_node<Key, Value> &leaf){//向左兄弟借东西
    ...
    if(leftleaf.numOfpair <= LEAST_PAIR_IN_LEAF_TICKET){ return false;}//左兄弟很贫穷
...
bool keep_node(internal_node<Key, Value> &node){
    ...
    if(node.numOfkey < LEAST_KEY_IN_INTER_TICKET){
...
bool borrow_from_rightnode(internal_node<Key, Value> &node){//从右侧节点借东西
    ...
    if(rightnode.numOfkey <= LEAST_KEY_IN_INTER_TICKET){//如果右边的兄弟贫穷了，就算了
        return false;
    }
    else{
...
bool borrow_from_leftnode(internal_node<Key, Value> &node){
    ...
    if(leftnode.numOfkey <= LEAST_PAIR_IN_LEAF_TICKET){//左边的很贫穷就算了
        return false;
    } else{
...
bool insert_in_leaf(leaf_node<Key, Value> &leaf, const Key &key, const Value &value){
    ...
    if(leaf.numOfpair == PAIR_IN_LEAF_TICKET){
...
void insert_in_node(internal_node<Key, Value> &node, const Key &key, const long &offset){
    ...
    if(node.numOfkey == KEY_IN_INTER_TICKET){
...
void spilt_leaf(leaf_node<Key, Value> &leaf){//将一个饱和的叶子分成两个,然后改变放入他的父亲中，如果父亲满了就向上分裂
    ...
    newleaf.numOfpair = (int)(PAIR_IN_LEAF_TICKET / 2 + 0.999);
...
void spilt_inter(internal_node<Key, Value> &node){//分裂节点
    ...
    if(node.offset == meta.root){//如果它是跟节点的话
        ...
        //spilt the node
        newnode.numOfkey = (int)(KEY_IN_INTER_TICKET / 2 + 0.999);
...
#endif //TRAINANDTICKET_BPLUSTREEFORTICKET_H
