...
#ifndef TRAINANDTICKET_BPLUSTREEFORSTATION_H
#define TRAINANDTICKET_BPLUSTREEFORSTATION_H
//
// Created by 金乐盛 on 2018/6/2.
//
...
class bplustree{
  ...
  public:
    ...
    bool erase(const Key &key){
        ...
        //if(key == debug) cout << "fuck0" << endl;
        if(leaf.numOfpair < LEAST_PAIR_IN_LEAF_STATION){
            //if(key == debug) cout << "fuck0" << endl;
  ...
  private:
    ...
    bool borrow_from_rightleaf(leaf_node<Key, Value> &leaf){//向右边叶子借东西
        ...
        if(rightleaf.numOfpair <= LEAST_PAIR_IN_LEAF_STATION){//如果右边叶子贫穷，那么就媒介成功
            return false;
        } else{
            //borrow from right
    ...
    bool borrow_from_leftleaf(leaf_node<Key, Value> &leaf){//向左兄弟借东西
        ...
        if(leftleaf.numOfpair <= LEAST_PAIR_IN_LEAF_STATION){ return false;}//左兄弟很贫穷
    ...
    bool keep_node(internal_node<Key, Value> &node){
        ...
        if(node.numOfkey < LEAST_KEY_IN_INTER_STATION){
    ...
    bool borrow_from_rightnode(internal_node<Key, Value> &node){//从右侧节点借东西
        ...
        if(rightnode.numOfkey <= LEAST_KEY_IN_INTER_STATION){//如果右边的兄弟贫穷了，就算了
            return false;
        }
        else{
    ...
    bool borrow_from_leftnode(internal_node<Key, Value> &node){
        ...
        if(leftnode.numOfkey <= LEAST_PAIR_IN_LEAF_STATION){//左边的很贫穷就算了
            return false;
        } else{
    ...
    bool insert_in_leaf(leaf_node<Key, Value> &leaf, const Key &key, const Value &value){
        ...
        if(leaf.numOfpair == PAIR_IN_LEAF_STATION){
    ...
    void insert_in_node(internal_node<Key, Value> &node, const Key &key, const long &offset){
        ...
        if(node.numOfkey == KEY_IN_INTER_STATION){
    ...
    void spilt_leaf(leaf_node<Key, Value> &leaf){//将一个饱和的叶子分成两个,然后改变放入他的父亲中，如果父亲满了就向上分裂
        ...
        newleaf.numOfpair = (int)(PAIR_IN_LEAF_STATION / 2 + 0.999);
    ...
    void spilt_inter(internal_node<Key, Value> &node){//分裂节点
        ...
        if(node.offset == meta.root){//如果它是跟节点的话
            ...
            //spilt the node
            newnode.numOfkey = (int)(KEY_IN_INTER_STATION / 2 + 0.999);
...
#endif //TRAINANDTICKET_BPLUSTREEFORSTATION_H
